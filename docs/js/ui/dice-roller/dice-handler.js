...
export function handleRoll() {
  ...
  if (mode === 'adv' && baseFormula.includes('d20')) {
    // Substitui o primeiro 'd20' encontrado
    finalFormula = baseFormula.replace(/(\d*)d20/, (match, qtyString) => {
      const qty = parseInt(qtyString) || 1; // Pega a qtd (ex: "2" em 2d20)
      // Rola 1 dado a mais, mas MANTÉM (Keep) apenas 1 (kh1)
      return `${qty + 1}d20kh1`;
    });
    ...
    finalFormula = baseFormula.replace(/(\d*)d20/, (match, qtyString) => {
      ...
      // Rola 1 dado a mais, mas MANTÉM (Keep) apenas 1 (kl1)
      return `${qty + 1}d20kl1`;
    });
  }
