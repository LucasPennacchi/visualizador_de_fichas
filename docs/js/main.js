  ...
  for (const data of charactersData) {
    ...
    if (data.error) {
      ...
      if (!cardElement.hasAttribute('data-rendered')) {
        // 1. Cria o HTML interno pela primeira vez
        grid.renderNewCardHTML(cardElement, data);
        cardElement.setAttribute('data-rendered', 'true');
      }
      // 2. SEMPRE roda o update
      // (Em cards novos, ele aplica as classes de status (dying/crazy))
      // (Em cards velhos, ele atualiza os dados)
