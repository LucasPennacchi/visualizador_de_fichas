  ...
  function handleDeleteLink(linkToDelete) {
    ...
    combat.removeCombatantByLink(linkToDelete);
  ...
  async function init() {
    ...
    // Define a nova classe 'card-grab-zone' como a única área que permite arrastar
    handle: '.card-grab-zone', 
    animation: 150, 
    ghostClass: 'sortable-ghost', 
    chosenClass: 'sortable-chosen',
    onEnd: function (evt) {
      if (document.body.classList.contains('combat-mode')) {
          combat.updateOrderFromDOM();
      } else {
          let links = store.getLinks(); 
